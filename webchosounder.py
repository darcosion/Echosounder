...
def scan_services():
    if not if_contain_cible(request.json):
        ...
        return jsonify(scan=echosounder.data_creation_services_discovery(request.json['cible'],
                                                                         port_start=request.json['port_start'],
                                                                         port_end=request.json['port_end']))
...
def scan_fast_services():
    if not if_contain_cible(request.json):
        ...
        return jsonify(scan=echosounder.retrieve_top_services())
...
def ip_to_as(ip):
    ...
    with app.open_resource('asinfo/routeviews-prefix2as-latest.json', 'r') as list_cidr:
        list_ip_cidr = [[ipaddress.IPv4Network(i[0]), i[1]] for i in json.loads(list_cidr.read())]
        list_cidr.close()
        for i in list_ip_cidr:
            if ip in i[0]:
